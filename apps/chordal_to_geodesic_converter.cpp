...
const std::string exe_name("chordal_to_geodesic_converter");
#define LOG std::cerr << FG_YELLOW(exe_name) << "|"
using namespace std;
using namespace srrg2_core;
using namespace srrg2_solver;

using VariableOutType = VariableSE3QuaternionRightAD;
using VariableInType  = VariableSE3EulerLeft;
using FactorOutType   = SE3PosePoseGeodesicErrorFactor;
using FactorInType    = SE3PosePoseChordalHessianFactor;
...
void convertGraph(FactorGraphPtr& out, const FactorGraphPtr& input) {
  ...
  for (const auto& id_factor : factors) {
    ...
    if (f_in) {
      ...
      f_out->setInformationMatrix(f_in->poseInformationMatrix());
