...
namespace srrg2_solver {
  ...
  SE3ChordalFactor::SE3ChordalFactor() : BaseType() {
  ...
  void SE3ChordalFactor::setInformationMatrix(const Matrix6f& omega) {
    _pose_omega = omega;
    _omega.setIdentity();
    ...
    Matrix6f sigma         = _pose_omega.inverse();
    InformationMatrixType sigma_chordal = Je * sigma * Je.transpose();
    Eigen::JacobiSVD<InformationMatrixType> svd(sigma_chordal,
                                                Eigen::ComputeThinU | Eigen::ComputeThinV);
    InformationMatrixType conditioned_sigma_chordal = InformationMatrixType::Zero();
    float conditioned_eigenvalue = 1.0;
    float threshold                                 = 1e-5;
    for (int i = 0; i < 12; ++i) {
      //! best
      if (svd.singularValues()(i, i) < threshold) {
        conditioned_eigenvalue = svd.singularValues()(i, i) + threshold;
      } else {
        conditioned_eigenvalue = svd.singularValues()(i, i);
      }
      conditioned_sigma_chordal.noalias() +=
        conditioned_eigenvalue * svd.matrixU().col(i) * svd.matrixU().col(i).transpose();
    }
    _omega = conditioned_sigma_chordal.inverse();
  }

  void SE3ChordalFactor::setMeasurement(const Isometry3f& measurement_) {
  ...
  void SE3ChordalFactor::compute(bool chi_only, bool force) {
  ...
  void SE3ChordalFactor::serialize(ObjectData& odata, IdContext& context) {
  ...
  void SE3ChordalFactor::deserialize(ObjectData& odata, IdContext& context) {
